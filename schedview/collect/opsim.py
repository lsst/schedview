...
def read_opsim(filename, start_time='2000-01-01', end_time='2100-01-01'):
    ...
    with sqlite3.connect(filename) as sim_connection:
        visits = pd.read_sql_query(
            f"SELECT * FROM observations WHERE observationStartMJD BETWEEN {start_mjd} AND {end_mjd}", sim_connection, index_col="observationId"
        )
    
    visits['start_date'] = pd.to_datetime(visits["observationStartMJD"] + 2400000.5, origin="julian", unit="D", utc=True)
