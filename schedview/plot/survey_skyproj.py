...
def _map_grid_columns(hpix_maps: np.array, num_rows: int) -> int:
...
def _map_grid_figure(num_rows: int, num_columns: int) -> Figure:
...
def _map_grid_bands(hpix_maps: np.array) -> list:
