...
def add_footprint_outlines_to_skymaps(footprint, spheremaps, colormap=None, filled=False, **line_kwargs):
    ...
    footprint_polygons = find_healpix_area_polygons(footprint)
    for spheremap in spheremaps:
        for region_name in footprint_polygons:
            if region_name == outside:
                continue
            for this_loop in footprint_polygons[region_name]:
    ...
    for hpix in np.arange(low_npix):
        ...
        for region in polygons_in_low_hpix:
            if region in polygon_data_sources:
                for polygon_coord_sequence in polygons_in_low_hpix[region]:
